...
def load_rois_from_image(filepath, some):
    global currentimage
    currentimage = some
    ...
    circle = [roi, pupil_circle, ext_iris_circle]
    
    names_file = open(some + '/bin.bin', 'wb')
    pickle.dump(circle, names_file)
    names_file.close()
...
def get_iris_boundaries(img, show=False):
    ...
    if show:
        ...
        with open('path.txt', 'w') as f:
            f.write(currentimage)
    
        cv2.imwrite(currentimage + '/iris boundaries.jpg', cimg)
        #ch = cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def get_mean_circle(circles, draw=None):
    ...
    if draw is not None:
        ...
        cv2.imwrite('/mean circle.jpg', draw)
        #ch = cv2.waitKey(0)
        cv2.destroyAllWindows() 
...
def filtered_circles(circles, draw=None):
    ...
    if draw is not None:
        ...
        global currentimage
        print(currentimage)
        cv2.imwrite(currentimage + '/filtered_circles() total={0} filtered_pos={1} filtered={2}.jpg'.\
                   format(len(circles), len(filtered_pos), len(filtered)),
                   draw)
        #ch = cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def get_equalized_iris(img, ext_iris_circle, pupil_circle, show=False):
    ...
    if show:
        ...
        global currentimage
        print(currentimage)
        cv2.imwrite(currentimage + '/equalized histogram iris region.jpg', roi)
        #ch = cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def get_rois(img, pupil_circle, ext_circle, show=False):
    ...
    rois['right-side']['ext_circle'] = \
            (0, int(int(1.25*ext_circle[2])), int(ext_circle[2]))
    rois['left-side']['ext_circle'] = \
            (int(int(1.25*ext_circle[2])), 
             int(int(1.25*ext_circle[2])), 
             int(ext_circle[2]))
    rois['bottom']['ext_circle'] = \
            (int(int(1.25*ext_circle[2])), 0, int(ext_circle[2]))
    ...
    rois['right-side']['img'] = \
        rois['right-side']['img'][0:int(2.5*ext_circle[2]), 0:int(1.25*ext_circle[2])]
    rois['left-side']['img'] = \
        rois['left-side']['img'][0:int(2.5*ext_circle[2]), 0:int(1.25*ext_circle[2])]
    rois['bottom']['img'] = \
        rois['bottom']['img'][0:int(1.25*ext_circle[2]), 0:int(2.5*ext_circle[2])]
    rois['complete']['img'] = \
        rois['complete']['img'][0:int(2.5*ext_circle[2]), 0:int(2.5*ext_circle[2])]

    if show:
        global currentimage
        plt.subplot(2,2,1),plt.imshow(rois['right-side']['img'], cmap='gray')
        plt.title('right-side'),plt.xticks([]),plt.yticks([])
        #plt.savefig(currentimage + "/right.jpg")
        #gray = cv2.cvtColor(rois['right-side']['img'], cv2.COLOR_BGR2GRAY)
        cv2.imwrite(currentimage + "/right.jpg", rois['right-side']['img'])
        plt.subplot(2,2,2),plt.imshow(rois['left-side']['img'], cmap='gray')
        plt.title('left-side'),plt.xticks([]),plt.yticks([])
        #plt.savefig(currentimage + "/left.jpg")
        #gray = cv2.cvtColor(rois['left-side']['img'], cv2.COLOR_BGR2GRAY)
        cv2.imwrite(currentimage + "/left.jpg", rois['left-side']['img'])
        plt.subplot(2,2,3),plt.imshow(rois['bottom']['img'], cmap='gray')
        plt.title('bottom'),plt.xticks([]),plt.yticks([])
        #plt.savefig(currentimage + "/bottom.jpg")
        #gray = cv2.cvtColor(rois['bottom']['img'], cv2.COLOR_BGR2GRAY)
        cv2.imwrite(currentimage + "/bottom.jpg", rois['bottom']['img'])
        plt.subplot(2,2,4),plt.imshow(rois['complete']['img'], cmap='gray')
        plt.title('complete'),plt.xticks([]),plt.yticks([])
        #plt.savefig(currentimage + "/complete.jpg")
        #gray = cv2.cvtColor(rois['complete']['img'], cv2.COLOR_BGR2GRAY)
        cv2.imwrite(currentimage + "/complete.jpg", rois['complete']['img'])
        #plt.show()
        print(currentimage)
...
def load_keypoints(sift, rois, show=False):    
    ...
    for pos in ['right-side','left-side','bottom','complete']:        
        ...
        for kp in rois[pos]['kp'][:]:
            ...
            if point_in_circle(rois[pos]['pupil_circle'][0],
                ...
                rois[pos]['kp'] = tuple(i for i in rois[pos]['kp'] if i is not kp)
                inside +=1
    ...
    if show:
        ...
        for pos in ['right-side','left-side','bottom']:
            ...
            cv2.imwrite(currentimage + "/" + str(pos) + "_keypoints.jpg", rois[pos]['img_kp_init'])
            plt.xticks([]), plt.yticks([])
            plt.subplot(3, 2, 2*i+2), \
            plt.imshow(rois[pos]['img_kp_filtered'])
            cv2.imwrite(currentimage + "/" + str(pos) + "_keypoints.jpg", rois[pos]['img_kp_filtered'])
            plt.xticks([]), plt.yticks([])
            i+=1
        #plt.show()
        print(currentimage)
...
def getall_matches(rois_1, rois_2, dratio,
    ...
    for pos in ['right-side','left-side','bottom','complete']:
        ...
        if show:
            ...
            cv2.imwrite(pos + '_matches.jpg', crt_image)
            #ch = cv2.waitKey(0)
            cv2.destroyAllWindows()
...
def load_rois_from_bin(bin_path):
    ...
    names_file = open(bin_path, 'rb')
    circle = pickle.load(names_file)
    names_file.close()
    
    roi, pupil_circle, ext_iris_circle = circle[0], circle[1], circle[2]
...
def unpickle_keypoints(array):
    keypoints = []
    for point in array:
        temp_kp = cv2.KeyPoint(x=point[0][0],y=point[0][1],_size=point[1], _angle=point[2], _response=point[3], _octave=point[4], _class_id=point[5]) 
...
def pickle_rois(rois, path):
    copyreg.pickle(cv2.KeyPoint().__class__, _pickle_keypoints)
    names_file = open(str(path) + '/bin.bin', 'wb')
    pickle.dump(rois, names_file)
    names_file.close()
...
if __name__ == "__main__":
    ...
    # Specify 2 image paths
    filepath1 = r'./001_2_1.jpg'
    filepath2 = r'./001_2_2.jpg'
    ...
    # Specify 2 image paths
    filepath1 = r'./001_1_1.jpg'
    filepath2 = r'./001_1_3.jpg'
